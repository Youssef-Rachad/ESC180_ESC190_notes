...
Python is an interpreted language which means that a python script is read block by block or line by line by the Python interpreter and commands are executed as they are read. Note that the Python interpreter itself is a program. 
...
#### Beyond this Course
Part of why this exercise's importance is the introduction to computer architecture. At a very basic level, it is beneficial to understand how a computer <i>really works</i> and these topics are of interest to you, later courses on designing and optimising such architectures are available! Another example is application specific computing used in embedded systems, FPGAs and the like.

Furthermore: current trends in the compute industry are increasingly showing more opportunities in hardware engineering or hardware adjacent systems to extract better performance (think AI accelerators and quantum computers) and reduce power consumption (think consummer electronics).
